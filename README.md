...
**Moody** est une application desktop de moodboard : un canvas infini où glisser-déposer des images, les organiser librement, zoomer, naviguer.
...
- Format de fichier `.moody` portable (un seul fichier, zéro dépendance)
...
La plupart des outils de moodboard sont des boîtes noires. Avec Moody, un script Python peut :
...
Les boards sont sauvegardés au format `.moody` — une archive ZIP contenant un fichier `board.json` et les images embarquées. Le format est ouvert et documenté pour permettre une utilisation depuis des scripts Python ou d'autres outils.

---

## Soutenir le projet

Moody est développé sur mon temps libre. Si le projet vous intéresse, vous pouvez le soutenir sur [Tipeee](https://en.tipeee.com/chez-jerem/).
